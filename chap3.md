...
需要说明的是，本层仅描述请求自身属性，不包含节点侧系统状态，如 GPU 显存剩余率 $\rho_{\text{mem}}$、排队请求数 $n_{\text{queue}}$ 等。尽管这些变量同样是 CBS 调度决策的必要输入，但其语义属于系统运行时上下文，而不属于任务本身的描述。将两者分离，有助于保持 MLWD 的语义一致性，即 MLWD 仅回答“该任务具有什么样的资源消耗模式”，而不涉及“任务到达时系统处于何种状态”。
